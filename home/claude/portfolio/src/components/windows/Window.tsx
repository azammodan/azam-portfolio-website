...
export default function Window({ id, title, children, className = '', minWidth = 260, minHeight = 180 }: Props) {
  ...
  return (
    ...
    >
      {/* Title bar */}
      <div
        className="flex h-9 flex-shrink-0 cursor-move select-none items-center gap-2 border-b border-[var(--desktop-border)] bg-[var(--win-header)] px-3 rounded-t-window"
        onMouseDown={onTitleBarMouseDown}
      >
        {/* Traffic lights */}
        <div className="flex items-center gap-1.5">
          <button
            className="win-dot h-3 w-3 rounded-full bg-[#ff5f56] transition-opacity hover:opacity-80 focus-visible:outline-none"
            onClick={() => closeWindow(id)}
            title="Close"
            aria-label="Close window"
          />
          <button
            className="win-dot h-3 w-3 rounded-full bg-[#ffbd2e] transition-opacity hover:opacity-80 focus-visible:outline-none"
            onClick={() => minimizeWindow(id)}
            title="Minimise"
            aria-label="Minimise window"
          />
          <button
            className="win-dot h-3 w-3 rounded-full bg-[#28c840] transition-opacity hover:opacity-80 focus-visible:outline-none"
            title="Maximise"
        ...
        {/* Title */}
        <span className="win-title flex-1 text-center text-xs font-medium text-[var(--desktop-muted)] mr-10 pointer-events-none">
          {title}
        </span>
      </div>
